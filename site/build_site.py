  ...
  $('#grid').innerHTML=filtradas.map(l=>{
    ...
    return `<div class="card">
      ...
      <div class="body">
        ...
        ${l.endereco||l.empreendimento?`<div class="custos">📍 ${[l.empreendimento,l.endereco].filter(Boolean).join(' — ')}${l.endereco_fonte?' <span title="localização deduzida pelo nome do condomínio">🏢✓</span>':''}${l.latitude?` · <a href="https://www.google.com/maps?q=${l.latitude},${l.longitude}" target="_blank" rel="noopener">ver ponto exato 🗺</a>`:''}</div>`:l.latitude?`<div class="custos">🗺 <a href="https://www.google.com/maps?q=${l.latitude},${l.longitude}" target="_blank" rel="noopener">ponto exato no mapa</a></div>`:''}
